...
| `ExecutionPath` | `FAST | FULL` | work item / bounded cycle | materialize only when the execution-control choice is decision-relevant; promote on new evidence | Independent from coordination baseline, assurance, and persistence. Routine bounded Master work may apply FAST semantics implicitly from the kernel without loading `master-cycle.md` or persisting/materializing the label. |
