...
| `REPOSITORY-MUTATION-SCOPE` | Repository mutation is limited to the explicit repository allowlist; relationships, dependencies, discovery, project membership, and technical access never widen it, ambiguous repository scope remains read-only until clarified, and required out-of-scope changes are handed off. | `authority-gates.md` | `SKILL.md` §2 Repository scope; `authority-gates.md` §1 and `CAN_EXECUTE(action)` | DQ |
