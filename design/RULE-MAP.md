...
| `FAST-FULL-SELECT` | FAST/FULL semantics follow ambiguity/dependency/review/control need; routine clear Master-only work may apply FAST behavior implicitly from the kernel without loading `master-cycle.md` or materializing the label. | `master-cycle.md` | `SKILL.md` §4-5 hot-path projection; `master-cycle.md:61-73`; `task-contract.md:23-36` | K, L, AB, CF, CM, DR |
...
| `DELEGATION-PROPORTIONAL` | Delegate only when specialization/throughput/parallelism materially helps; among safe assignments optimize expected finished critical path after dispatch/review/reconciliation overhead, preferring lower coordination/risk when the difference is marginal; bounded single delegation can remain LIGHTWEIGHT while still using full Worker envelope. | `master-cycle.md` | `SKILL.md:27`; `master-cycle.md:74-85`; `worker-protocol.md:58-60` | Q, AN |
