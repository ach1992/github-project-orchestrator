...
| `MACHINE-RELAY-PORTABLE` | AI-to-AI relay prose is English by default, identity-bearing/decision-relevant literals stay exact unless safety/redaction requires otherwise, and human-relayed payloads become one complete copy target while domain owners retain payload semantics and no workflow state/control is weakened. | `SKILL.md` human-relay kernel | post-v1.2.0 requirement: `docs/PROJECT-SPEC.md` §3.2 | AT, BC, CK, DI |
