...
| `FAST-FULL-SELECT` | FAST/FULL semantics follow ambiguity/dependency/review/control need; routine clear Master-only work may apply FAST behavior without loading `master-cycle.md` or materializing the label. | `master-cycle.md` | `SKILL.md` §4-5; `master-cycle.md` §4; `task-contract.md` §2 | K, L, AB, CF, CM |
...
| `SYNTHESIZE-WORK` | Outcome incomplete + no READY item triggers refine/unblock/split/investigate, not automatic NO_READY_WORK; phase decomposition starts from current residual scope rather than speculative seams. | `master-cycle.md` | `SKILL.md:93-94`; `master-cycle.md` §8; `governance.md` §3 | O, AX, DL, DO |
