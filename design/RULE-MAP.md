...
| `WORKSPACE-REUSE-HYGIENE` | Reuse a suitable existing repository/worktree, prefer `git worktree` when branch/task isolation is sufficient, create a separate full clone only for genuine repository/tooling isolation, and remove only explicitly task-owned disposable resources without risking useful, ambiguous, or unrelated state. | `SKILL.md` mutation + safety invariants | post-v1.3.3 requirement: Issue #96 | X, AI, BG, DL |
