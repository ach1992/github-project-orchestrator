...
| `MUTATION-IDEMPOTENT` | Discover/reuse/update/create-only-if-absent/verify; for repository/workspace state, reuse a suitable existing repository/worktree and prefer `git worktree` when branch/task isolation is sufficient; incomplete discovery is not absence. | `SKILL.md` mutation invariant | `SKILL.md:43`; post-v1.3.3 requirement: Issue #96 | B, AU, BG, DL |
...
| `PROTECT-UNRELATED` | Never destroy/absorb unrelated user/contributor work; remove task-created temporary resources only when ownership is explicit, purpose has ended, and no useful, ambiguous, or unrelated state can be lost. | `SKILL.md` safety invariant | `SKILL.md:50`; post-v1.3.3 requirement: Issue #96 | X, AI, DL |
