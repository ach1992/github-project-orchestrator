...
Use the least-fragile question-specific authoritative capability: native/structured identity-verified GitHub state for management; local Git + fresh refs for commit/ref/diff/worktree truth; current CI/check objects/logs tied to SHA; actual release/deployment platform tied to immutable artifact/commit identity; authoritative repository content; official primary docs/specs/release notes/security advisories for version-sensitive external contracts. Do not choose weaker sources for convenience. If preferred capability is unavailable, use an alternative only when identity/semantics are preserved and cross-check identities when combining sources. One route failure is route-specific, not proof of missing capability; distinguish transient operation failure from established route unavailability. Do not re-probe unchanged failure conditions when another valid path exists; re-check only when new evidence makes success plausible or explicitly transient failure semantics justify a bounded retry. Keep capability conclusions runtime-transient; create no tool-inventory artifact. Bundled scripts are optional read-only accelerators, never mandatory gates. Incomplete helper evidence means unknown, not absent/clean; inspect authoritatively only when the missing evidence can affect next action.
