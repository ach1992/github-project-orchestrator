...
| Repository scope | Treat repository mutation authority as an explicit allowlist. Mutate only repositories the owner/assignment clearly authorized for this Master/Worker; one clear repository assignment is a singleton scope, and a clear multi-repository grant includes only those repositories. Mere mention, dependency, link, technical access, coordination, or discovery never widens this scope. If writable repository scope is materially ambiguous, perform only read-only reconciliation and ask the smallest exact scope question before mutation. |
...
4. never reprioritize, broaden scope or repository mutation scope, merge/integrate the target, release, or upgrade ProjectAuthority/ScopedAuthorization/CoordinationBaseline/AssuranceLevel.
