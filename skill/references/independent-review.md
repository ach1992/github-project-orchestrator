...
If direct reviewer tooling is unavailable but a fresh independent chat/model/human is usable, the Master must emit one complete ready-to-paste `INDEPENDENT REVIEW CHAT` MachineRelay in the current user-visible response for user-mediated dispatch, then reconcile the returned result. Durable GitHub/PR/Issue state may supply authoritative evidence/locators inside that prompt, but it never substitutes for emitting the prompt; do not replace the relay with a GitHub-only pointer or instructions for the next chat to reconstruct the review request. Lack of a GitHub reviewer username alone is not a blocker.
