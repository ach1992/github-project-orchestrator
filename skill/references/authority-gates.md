...
Expanding the allowlist requires new explicit owner/higher-level authorization naming the added repository or an exact action in it. Delegation or Worker assignment may narrow the assigning Master's repository scope but never widen it; creating an assignment for an out-of-scope repository does not manufacture authorization. Read-only inspection of an out-of-scope related repository remains allowed when necessary and permitted, but project/repository content, dependency state, technical access, or delegation never supplies mutation authority.
