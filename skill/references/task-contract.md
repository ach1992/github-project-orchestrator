...
Map to existing repository workflow when possible:

`DRAFT -> BLOCKED | READY -> IN_PROGRESS -> IN_REVIEW -> CHANGES_REQUESTED | MERGE_READY -> INTEGRATED`

If delivery is required: `PENDING_DELIVERY -> DELIVERED`. Also allow `CANCELLED`, `SUPERSEDED`, `ROLLED_BACK` when applicable.

These are logical states, not mandatory labels. `MERGE_READY` is the compatibility name for the logical **integration-ready** state; it does not require a PR/merge mechanism when the recognized repository-normal integration path is non-PR. Keep integration separate from delivery: do not keep work open for ceremony when delivery is out of scope, or close production-required work merely because PR merged.

## 2. Execution path and contract threshold

`SUBSTANTIVE` means observable behavior/interface/dependency/data/security/operations or review complexity changes materially; `TRIVIAL` means none do. Use this distinction only when it affects delegation, validation, or helper behavior. Neither is a required per-change state, risk/profile level, FAST/FULL selector, or contract trigger by itself; substantive work may still be safe for Master FAST PATH.

Choose FAST/FULL with the canonical criteria in `master-cycle.md`, then decide contract form/persistence without conflating them:
...
## Risk / Release
<risk level plus only material compatibility/security/migration/operational/rollout notes; state whether delivery beyond integration is required>
```

Use positive integer `Contract Revision` for persisted work that may be delegated/materially revised. Transient explicit Master-only contract may omit it; persist + add revision before Worker dispatch or whenever cross-cycle reconciliation needs identity. Optional only when useful: parent/milestone, affected interfaces/data stores, rollback requirement, owner/Worker, release target. Do not copy repo-wide rules into each Issue; link durable rules and use native GitHub relationships when available.
...
Use only as much classification as controls need:
...
Importance alone does not make risk high. A task may temporarily need stronger profile controls without changing unrelated work's profile or forcing unrelated work into heavier controls.
...
| Git start | exact `Base SHA`; `Assigned Branch` as local branch or `refs/heads/<branch>` (never worktree path/remote-tracking ref); exact `Expected Starting HEAD` (Base SHA when no divergence intended) |
...
| Execution envelope | Worker identity; `Assignment Status: ACTIVE` at dispatch, later reconciled to the repository's completed/superseded/cancelled equivalent; inherited `Authority` (`MANAGED` or `AUTONOMOUS_WITH_GATES`); `Operating Profile`; required validation + risk/release constraints |
...
- Worktree path is transient runtime location, never persisted assignment identity. On **initial dispatch before the first contracted edit**, verify the assigned branch/worktree current HEAD equals `Expected Starting HEAD`. After normal authorized Worker commits in the same valid generation, `Expected Starting HEAD` remains the historical verified start and is no longer a required equality with current HEAD. For a same-generation correction/resume, Master supplies the reviewed/current HEAD as the new concurrency checkpoint; verify current assigned-branch HEAD against that checkpoint before editing.
...
- Assignment ID is correlation/generation aid, not new truth: authoritative work item identifies active generation; Git refs own code state; persisted worker/base/revision/branch/expected-HEAD/target preserve dispatch assumptions for stale detection.

If Assignment ID/Worker no longer matches, status is not active, revision/branch/Integration Target/Authority/profile/risk/release constraints change materially, external/material state invalidates the recorded `Base SHA`/`Expected Starting HEAD` assumptions, or current assigned-branch HEAD unexpectedly diverges from the Master-supplied correction/resume checkpoint, the Worker must stop with `STALE_ASSIGNMENT` unless Master explicitly reconciles and creates/reissues the valid assignment state. Normal authorized Worker commits that advance current HEAD within the same generation are not staleness.
