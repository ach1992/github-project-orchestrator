...
Frame the requested work around defensive analysis, root cause, remediation, and verification. Do not solicit, disclose, or relay raw secrets/credential values; when authorized work legitimately requires credentialed access, use existing approved secret/runtime mechanisms without exposing those values. Do not request unrelated third-party targeting, unapproved production mutation, weaponization, persistence, evasion, or other unnecessary offensive action.

For an independent/read-only reviewer relay, default evidence acquisition to authoritative source/diff, repository-owned existing tests, current CI/log/artifact evidence, and safe read-only inspection. Do not ask the reviewer to invent, generate, mutate, or execute novel adversarial payloads/probes solely to prove that a security control can be bypassed or can resist a bypass. If the existing inspectable evidence does not prove a required security property, the reviewer should report an evidence-backed `BLOCKER`/`REQUIRED` finding or `INCOMPLETE / NOT_ISSUED` as appropriate instead of manufacturing a new probe. This reviewer boundary does not prohibit bounded defensive regression tests during explicitly scoped implementation/remediation when the accepted task requires them, the action is authorized and isolated/reversible, and provider/platform policy permits it; repository-owned existing security tests may still be inspected or run when otherwise allowed.

If a provider/tool restricts one detail, omit or redact only that detail, continue every safely allowed outcome-linked review/remediation/test, and report the exact limitation plus its effect on evidence/completeness. Do not replace all safe work with an undifferentiated refusal merely because security is involved. If a limitation prevents required review evidence, return an incomplete review without approval under `review-integration.md`; never fabricate evidence or weaken security to avoid the limitation.
...
|---|---|
| Master / self-execution | FAST keeps concern handling transient when request + repository evidence already make implementation/validation clear. FULL/persisted work records only concern-derived facts that materially affect existing Goal/Scope/Acceptance/Validation/Risk/Release fields; never add a generic `EngineeringConcerns` field. Implement the smallest repository-consistent solution that addresses the selected failure modes and validate the concern at the narrowest high-signal boundary first, then broader required checks. If a still-material concern fact is not recoverable from stronger code/Git/GitHub/CI/release evidence, persist only that missing unresolved fact through `continuity.md` in its natural owner. |
| Worker | Apply concern requirements already in current contract/acceptance/validation/special constraints plus directly evident in-scope correctness/safety obligations. Do not perform repository-wide observability/CI/privacy/accessibility/reliability redesign. A newly discovered concern that materially changes scope, acceptance, risk, architecture/security/privacy posture, migration, or release expectations uses the existing Worker stop/contract-revision path rather than silently broadening the assignment. Master dispatches only the smallest actionable concern requirement in existing Acceptance/Validation/Special constraints; do not send the entire concern catalog when only one or two concerns matter. |
