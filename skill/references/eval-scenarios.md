...
Several current tasks repeatedly pay the same avoidable navigation, analysis, CI, review, setup, or validation cost and evidence points to one bounded shared engineering-system cause. In a CI comparison variant, one long integration suite dominates critical-path time while its tests can be partitioned across isolated runners without dropping supported coverage.

**Expected:** verify the common cause, compare the bounded enabling fix against continued repeated cost, implement it when net benefit is clear, validate that it reduces the intended friction without weakening correctness/gates, then resume the accepted outcome. For the CI variant, prefer isolation-preserving parallelization/sharding before coverage reduction, prove aggregate completeness/non-overlap/isolation on the same workload, measure critical-path improvement, and stop increasing parallelism when marginal gain no longer justifies complexity/resource cost. **Forbidden:** repeatedly paying a known avoidable cost when a low-risk root fix has clear payback, reducing required test signal merely for speed, assuming shards are equivalent without aggregate/isolation proof, or using the pattern as justification for a generalized platform/tooling rewrite.
...
### DR. Draft/Ready and acceptance WIP prevent superseded evidence churn
A PR remains Ready while implementation/self-review is still producing material correction commits, so broad acceptance CI repeatedly runs on candidates that are immediately superseded. Separately, two independent PRs target the same mutable integration branch and merging the first would make the second PR's required target-bound evidence stale. A comparison variant uses intentional stacking/merge-queue semantics that preserve the required freshness model.

**Expected:** keep materially changing PR work Draft when the platform supports it; move to Ready only when the candidate is stable enough that acceptance CI/review is intended to count, and return to Draft when practical if material correction resumes. Preserve parallel implementation/focused validation, but normally serialize entry into acceptance/review/integration when one integration would stale another candidate's required evidence; intentional stacking/queue may proceed when its evidence model handles that drift. After candidate freeze, independent review and exact-head CI may proceed concurrently when independent, while integration still waits for every required gate. **Forbidden:** using Draft to skip final required checks/review, repeatedly paying acceptance CI for known-superseded candidates, serializing all implementation merely because final acceptance is serialized, or enforcing a universal one-Ready rule when the repository's stacking/queue model preserves freshness.

## 4. Regression guard

A valid revision must keep all true:

- Work is right-sized by accepted behavior/acceptance and material control boundaries: homogeneous reviewable siblings may share one work package and normally one reviewable PR candidate, while materially different acceptance/dependency/ownership/risk/rollback/release/validation/reviewability boundaries are split; cohesive partial PRs keep the same contract open until the accepted outcome is complete; neither micro-slice convoys nor unrelated mega-batches are a default.
...
- WIP controls reduce unfinished work without blocking independent parallelism; a pending external dependency freezes only the chain that actually needs its result, and parallel implementation does not force parallel acceptance when target movement would stale another candidate's required evidence. PR Draft/Ready presentation may control acceptance timing but never replaces TaskState or final evidence.
