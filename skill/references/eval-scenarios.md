...
**Expected:** verify the common cause, compare the bounded enabling fix against continued cost, implement it when net benefit is clear, validate that correctness/gates remain intact, then resume product work. For the long-suite variant, prefer isolation-preserving parallelization/sharding before coverage reduction, prove complete/non-overlapping isolated aggregate coverage on the same workload, measure critical-path gain, and stop at diminishing returns. For the changing-PR variant, use Draft while material correction continues when the platform supports it; mark Ready when acceptance CI/review is intended to count, returning to Draft if material correction resumes. Draft/Ready is presentation, not a substitute for final required evidence. **Forbidden:** repeatedly paying a known avoidable cost, reducing required signal for speed, assuming shard equivalence without proof, using Draft to skip final gates, or turning the pattern into a generalized tooling/process rewrite.
...
- WIP controls reduce unfinished work without blocking independent parallelism; target-bound acceptance may serialize when one integration would stale sibling evidence, while PR Draft/Ready presentation may control acceptance timing but never replaces TaskState or final evidence.
