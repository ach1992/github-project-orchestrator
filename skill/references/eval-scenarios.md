...
**Expected:** sequence unstable edits or use explicit safe stacking. Preserve independent implementation where useful, but normally serialize acceptance/review/integration when target movement would stale sibling evidence unless the repository's intentional stacking/queue model preserves freshness. **Forbidden:** parallel dispatch merely for utilization, serializing all independent implementation just because acceptance is serialized, or knowingly creating avoidable stale-evidence churn.
...
Several current tasks repeatedly pay the same avoidable navigation, analysis, CI, review, setup, or validation cost and evidence points to one bounded shared engineering-system cause. Comparison variants include a long integration suite that can be isolated across runners without dropping coverage, and a PR kept Ready while material correction commits repeatedly supersede broad acceptance CI.

**Expected:** verify the common cause, compare the bounded enabling fix against continued cost, implement it when net benefit is clear, validate that correctness/gates remain intact, then resume product work. For the long-suite variant, prefer isolation-preserving parallelization/sharding before coverage reduction, prove aggregate completeness and isolation plus non-overlap where relevant on the same workload, measure critical-path gain, and stop at diminishing returns. For the changing-PR variant, materially changing work may remain Draft when the platform supports it; use Ready when acceptance CI/review is intended to count, returning to Draft on resumed material correction only when useful to avoid stale acceptance work. Draft/Ready is optional presentation, not a substitute for final required evidence. **Forbidden:** repeatedly paying a known avoidable cost, reducing required signal for speed, assuming shard equivalence without proof, using Draft to skip final gates, or turning the pattern into a generalized tooling/process rewrite.
...
- Work is right-sized by accepted behavior/acceptance and material control boundaries: homogeneous reviewable siblings may share one work package and normally one reviewable PR candidate, while materially different acceptance/dependency/ownership/risk/rollback/release/validation/reviewability boundaries are split; cohesive partial PRs keep the same contract open and do not close/recreate it for mechanical seams, while legitimate completion/cancellation/supersession may terminate it; neither micro-slice convoys nor unrelated mega-batches are a default.
...
- WIP controls reduce unfinished work without blocking independent parallelism; target-bound acceptance may serialize when one integration would stale sibling evidence, while optional PR Draft/Ready presentation may signal intended acceptance timing but never replaces TaskState or final evidence.
