...
The sole remaining dependency is an already-running CI/check/deployment/job whose authoritative status is still pending.

**Expected:** finish independent work first; when a transition is plausibly due, perform at most one additional cheap immediate status re-read. If the still-pending external dependency is then the sole remaining blocker, use canonical `BLOCKED` with the exact object/resume condition unless another boundary more precisely describes the cause. Treat `pending` as a lifecycle/dependency state, never as `NO_READY_WORK` or an invented terminal label. **Forbidden:** tight polling, sleeping indefinitely, background-work claims, treating pending as failure, or stopping under the wrong boundary merely because the job is not finished.
...
### AT. Copyable prompt handoff
The user asks the Master to give them a prompt, relay prompt, or ready-to-paste instructions for another reviewer/Worker/chat.

**Expected:** return the complete ready-to-paste prompt as exactly one fenced code block with no unsolicited prose around it; preserve all engineering/safety instructions, and use a longer outer fence if embedded fences are required. **Forbidden:** prose-rendering the prompt as ordinary paragraphs, splitting it across multiple copy targets, or weakening the prompt merely to simplify formatting.
...
**Expected:** hand off repository/change identity, exact target/base and HEAD identities, acceptance/contract revision, risk/profile, review boundary/constraints, current validation evidence, bounded reviewer authority, and evidence-backed `BLOCKER`/`REQUIRED`/`OPTIONAL` output requirements. The Master independently reconciles returned findings and retains integration ownership. **Forbidden:** inventing a permanent reviewer state machine, transferring merge ownership, or using a context-poor prompt that relies on old chat narrative.
...
**Expected:** keep lifecycle state `PENDING_DELIVERY`, persist the exact completion condition, continue any independent useful work, and declare `DELIVERED` only when required delivery evidence becomes current and satisfied. If that delayed external evidence becomes the sole remaining dependency, terminal handling normally uses canonical `BLOCKED` with the exact evidence/resume condition; `PENDING_DELIVERY` is not itself a stop label and the case is not `NO_READY_WORK`. **Forbidden:** equating deployment success with delivery completion or confusing lifecycle state with canonical terminal boundary.
...
- WIP controls reduce unfinished work without blocking independent parallelism.
...
- Explicit prompt/relay-output requests produce one ready-to-paste fenced code block without changing the underlying engineering instructions or workflow semantics.
...
- The root project specification is project-level intent, not hot-path operational state: normal execution and Workers use the nearest current authoritative task/docs/code/GitHub/CI/release sources, and the root spec is reread only when materially decision-relevant.
...
- Independent-review handoffs are bounded and evidence-addressable without creating a new permanent role/state model; the Master retains acceptance/integration ownership.
...
- Deployment success is not `DELIVERED`; delayed required acceptance remains lifecycle state `PENDING_DELIVERY` with an exact completion condition and, when it is the sole external dependency, normally yields canonical `BLOCKED` rather than a fabricated pending/`NO_READY_WORK` stop. Wrong/unknown production artifact identity freezes rollout and enters incident/containment behavior when unintended state may be hazardous.
