...
While implementing accepted work, the Master discovers (a) a better in-scope implementation, including a variant where current evidence shows an internal boundary is itself the root cause and a bounded structural correction would remove it, (b) a material adjacent improvement outside the accepted outcome, and (c) cosmetic/speculative cleanup.

**Expected:** choose (a) when its value justifies cost/risk; in the internal-boundary variant, make only the bounded structural correction needed to remove the root cause while preserving compatibility obligations required by accepted scope, rather than preserving the harmful boundary merely because it exists; if accepted requirements themselves change a durable architecture boundary or public/external contract, the normal requirement-change path still applies. Propose or reuse worthwhile tracking for (b) without implementing it automatically, and ignore/reuse existing tracking for (c). Immediate correctness/security/data/production threats follow the existing risk/incident path. **Forbidden:** silently expanding the accepted outcome, preserving a demonstrated harmful internal boundary solely because it already exists, using the variant to justify broad redesign, auto-implementing every improvement, or generating backlog volume as proof of proactivity.
