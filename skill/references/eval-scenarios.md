...
### DR. Routine Master hot path skips unneeded orchestration-reference loading
A Master receives a clearly bounded low/medium-risk Master-only change whose outcome, acceptance, validation, dependencies, and rollback are already clear from the accepted request plus current repository evidence. There is no material migration/data/security boundary, production/release coordination, cross-item coordination, delegation, or unresolved material decision. A comparison variant introduces material dependency coordination or Worker delegation.

**Expected:** in the routine variant, execute the kernel's direct path from task/repository evidence without loading `master-cycle.md`, materializing `ExecutionPath=FAST`, or creating a Task Contract solely to classify the work; still load every independently triggered domain, including `authority-gates.md` before consequential mutation, and preserve validation/review/integration/continuation guarantees. In the comparison variant, load `master-cycle.md` and the other directly triggered coordination/Worker references before the affected decision. **Forbidden:** forcing `master-cycle.md` into every Master turn merely to name FAST/FULL, or using the direct path to bypass a triggered authority, coordination, Worker, review, release, recovery, or terminal-decision rule.
...
- Routine clear Master-only work may apply the direct FAST-equivalent kernel path without loading `master-cycle.md` or materializing an execution-path label solely for classification; any separately triggered authority/coordination/Worker/review/release/recovery/terminal domain still loads and retains its full guarantees.
