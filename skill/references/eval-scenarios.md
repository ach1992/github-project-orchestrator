...
**Expected:** write relay prose in English unless the user explicitly requests another relay language; preserve identity-bearing or decision-relevant literals such as identifiers, refs/SHAs, paths, commands, code/error strings, and quoted source-language/user-facing text when exact wording is material, unless an existing safety/redaction rule requires omission or redaction; when presented for copy/paste, return the complete domain-specific payload as exactly one fenced code block with no unsolicited prose around it, preserve all engineering/safety semantics, and use a longer outer fence when embedded fences require it. Direct user-facing explanation outside the relay remains in the user's language. **Forbidden:** prose-rendering a relay as ordinary paragraphs, splitting it across multiple copy targets, silently translating/normalizing identity-bearing or material source literals merely to make the relay English, silently weakening fields or instructions, making every user-facing answer English, or creating a new lifecycle/state merely for transport.
...
- Machine-relay prose uses English by default (unless explicitly overridden), identity-bearing/decision-relevant literals remain exact unless safety/redaction requires otherwise, and copy/paste relays produce one complete fenced code block without changing domain payload ownership, underlying engineering instructions, user-facing language, or workflow/state semantics.
