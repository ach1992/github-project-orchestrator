...
- right-size work to a **minimum meaningful slice**: use one work package when sibling changes share the same accepted behavior/acceptance boundary and materially aligned dependency, ownership, risk, rollback, release, and validation boundaries and the combined effective diff remains reviewable. When PR-based, default one such slice to one reviewable candidate/acceptance cycle; use partial PRs only when material sequencing, control, rollback/release, ownership, or reviewability boundaries require them—not merely for implementation layers. Do not fragment one homogeneous outcome into a convoy of mechanically similar Issue/PR/CI/review cycles merely because each seam can be isolated;
- preserve parallel implementation on genuinely independent surfaces, but when candidates share a mutable integration target and integrating one would stale another's required evidence, normally let only one enter acceptance/review/integration at a time unless intentional stacking/queue semantics preserve freshness;
