...
Decision order: first protect correctness/isolation, then compare expected finished-throughput gain with dispatch/review/reconciliation cost. When several assignments are safe, prefer the one with the lower expected implementation critical path after coordination overhead; when the difference is marginal or uncertain, prefer the lower-coordination/lower-risk assignment rather than manufacturing parallelism. Never delegate merely to keep Workers busy, and never withhold useful parallelism merely because Master could eventually do everything alone. Priority, acceptance, risk acceptance, contract change, integration approval, and release authorization remain Master-owned.
