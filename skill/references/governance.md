...
Use epics/parent Issues only when the milestone is too broad to coordinate directly. Before decomposing a new phase/milestone—or resuming it after predecessor integration—reconcile its completion gate against current integrated repository state and derive only the **residual meaningful outcomes/dependencies** needed for execution. Do not pre-create a speculative seam-level child backlog from a specification when current evidence may already satisfy part of the scope.
...
If one outcome spans multiple repositories/services, keep one coherent cross-repository outcome/release view while preserving each repository's own rules, Issues/PRs, CI, and ownership. The cross-repository view should contain only the global outcome/completion model, material cross-repository dependencies, release/integration order, and ownership/handoff links needed to coordinate them. Coordination never grants mutation authority: this Master may change only repositories inside its explicit `RepositoryMutationScope`; required work in a related out-of-scope repository is a handoff/dependency for that repository's authorized Master/owner. Surface the exact target repository and required change/dependency; when a user-mediated agent handoff is useful, provide the smallest sufficient MachineRelay under the canonical `SKILL.md` transport contract rather than mutating that repository. Keep implementation status and local backlog in each repository's natural authoritative sources; do not create a duplicate central backlog that mirrors every repo.
