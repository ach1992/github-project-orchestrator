...
This directory provides repeatable operational evaluation for the refactor against the immutable `v1.0.0` baseline.
...
Token/word/line size is diagnostic only. The scorer can report `SKILL.md` entrypoint size from Git when run with `--repo-root`, but entrypoint shrinkage cannot compensate for a protected-behavior regression.
...
- `traces-current.json` — refactored source-grounded traces pinned to current runtime paths.
