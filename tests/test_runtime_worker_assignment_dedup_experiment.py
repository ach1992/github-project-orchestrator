...
def main() -> None:
    ...
    required = (
        ...
        "path is runtime location, never assignment identity",
        "[task-contract.md](task-contract.md) §8 owns the persisted Worker assignment/concurrency envelope",
        "verify repository/working directory, assigned branch/worktree safety, repository rules, required validation, and task risk/release constraints",
        "verify the current persisted assignment envelope from §8",
        "on initial dispatch before the first contracted edit, require current assigned-branch/worktree HEAD = immutable `Start HEAD`",
        "later authorized same-generation commits may advance beyond it without staleness",
        "on same-generation correction/resume, require current assigned-branch HEAD = Master-supplied `Checkpoint HEAD` before editing",
